...
| Device Service | `deviceUuid` records, immutable canonical `deviceId`, current organization/lifecycle, ownership version/history, claim state, tank association, active profile reference, hardware/firmware state |
...
| Telemetry Service | Samples with device UUID/canonical ID and organization/version at ingest, batches/idempotency, aggregates, latest readings |
...
Ownership transfer updates the Device Service mapping atomically and appends history; it does not rewrite Telemetry Service rows. Normal telemetry history is filtered by the caller's trusted current organization and `organizationIdAtIngest`, so a new owner receives new/live telemetry without accidental access to pre-transfer rows. A broader audit-history access policy remains explicitly unapproved. See [ADR-018](../decisions/ADR-018-dual-device-identity.md).
